...
describe('ExpertViewUtils', () => {
  it('should properly convert expert drawer variant into value', () => {
    ...
    const [openSize, invertedOpenSize] = ExpertViewUtils.convertExpertViewModeToValue('open', defaultSize)
    expect(openSize).toEqual(defaultSize)
    expect(invertedOpenSize).toEqual('70%')

    const [hiddenSize, invertedHiddenSize] = ExpertViewUtils.convertExpertViewModeToValue('hidden')
    expect(hiddenSize).toEqual('0%')
    expect(invertedHiddenSize).toEqual('100%')

    const [fullscreenSize, invertedFullscreenSize] = ExpertViewUtils.convertExpertViewModeToValue('fullscreen', defaultSize)
