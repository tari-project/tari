...
use minotari_mcp_common::{
    ...
    get_required_string_param, get_required_u64_param, get_optional_string_param
};
...
use minotari_app_grpc::tari_rpc::{
    ...
    CancelTransactionRequest, CreateBurnTransactionRequest, UserPaymentId, BlockHeight, BlockHashHex,
...
impl McpTool for GetTransactionInfoTool {
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        let response = client.get_transaction_info(request).await
        ...
        let total_amount: u64 = transactions.iter()
            .filter_map(|tx| tx["amount"].as_u64())
            .sum();
        let total_fees: u64 = transactions.iter()
            .filter_map(|tx| tx["fee"].as_u64())
            .sum();
        let confirmed_count = transactions.iter()
...
impl McpTool for GetCompletedTransactionsTool {
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        let block_height = params.get("block_height").and_then(|v| v.as_u64()).map(|h| BlockHeight { block_height: h });
        ...
        let mut client = (*self.grpc_client).clone();
        let mut response_stream = client.get_completed_transactions(request).await
        ...
        let inbound_count = transactions.iter()
            .filter(|tx| tx["direction"].as_str() == Some("INBOUND"))
            .count();
        let outbound_count = transactions.iter()
            .filter(|tx| tx["direction"].as_str() == Some("OUTBOUND"))
            .count();
        
        let total_inbound: u64 = transactions.iter()
            .filter(|tx| tx["direction"].as_str() == Some("INBOUND"))
            .filter_map(|tx| tx["amount"].as_u64())
            .sum();
        let total_outbound: u64 = transactions.iter()
            .filter(|tx| tx["direction"].as_str() == Some("OUTBOUND"))
            .filter_map(|tx| tx["amount"].as_u64())
            .sum();
        let total_fees: u64 = transactions.iter()
...
impl McpTool for TransferTool {
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        let successful_transfers = results.iter()
...
impl McpTool for CoinSplitTool {
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        let mut client = (*self.grpc_client).clone();
        let response = client.coin_split(request).await
...
impl McpTool for CancelTransactionTool {
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        let mut client = (*self.grpc_client).clone();
        let response = client.cancel_transaction(request).await
...
impl McpTool for TransactionAnalysisTool {
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
        ...
        let limit = params.get("limit")
            .and_then(|v| v.as_u64())
            .unwrap_or(1000); // Analyze up to 1000 transactions
        ...
        // Calculate averages
        let avg_inbound = if !inbound_txs.is_empty() { total_inbound / inbound_txs.len() as u64 } else { 0 };
        let avg_outbound = if !outbound_txs.is_empty() { total_outbound / outbound_txs.len() as u64 } else { 0 };
        let avg_fee = if !outbound_txs.is_empty() { total_fees / outbound_txs.len() as u64 } else { 0 };
        
        // Transaction frequency analysis
        let transactions_per_day = total_transactions as f64 / days_back as f64;
        
        // Fee analysis
        let fee_rates: Vec<u64> = outbound_txs.iter()
            .filter(|tx| tx.fee > 0)
            .map(|tx| tx.fee)
            .collect();
