    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "payment_id": {
                    "type": "string",
                    "description": "Payment ID as a string or hex value (with 0x prefix)"
                }
            },
            "required": ["payment_id"]
        })
...
impl McpTool for AddressValidationTool {
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "address": {
                    "type": "string",
                    "description": "Tari address to validate (hex, base58, or emoji format)"
                }
            },
            "required": ["address"]
        })
    ...
    async fn execute(&self, params: Value) -> McpResult<Value> {
                    ...
                    match bytes.len() {
                        ...
                        _ => {
                            ...
                            validation_results.push(format!(
                                "Invalid address length: {} bytes (expected 32 or 33)",
                                bytes.len()
                            ));
                        },
                    }
        ...
        if !is_valid {
            ...
            match format_type {
                "HEX" => {
                    recommendations.push("Hex addresses are valid but Base58 format is more user-friendly".to_string())
                },
                "BASE58" => recommendations.push("Base58 format is recommended for sharing with users".to_string()),
                "EMOJI" => {
                    recommendations.push("Emoji format is fun but verify carefully before sending funds".to_string())
                },
                _ => {},
            }
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "address": {
                    "type": "string",
                    "description": "Tari address to convert"
                },
                "target_format": {
                    "type": "string",
                    "description": "Target format: hex, base58, emoji, or all",
                    "enum": ["hex", "base58", "emoji", "all"],
                    "default": "all"
                }
            },
            "required": ["address"]
