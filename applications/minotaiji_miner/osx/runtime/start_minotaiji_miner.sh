...
echo "${exe_path}/minotaiji_miner" ${init_flag} --config="${config_path}/config.toml" --log_config="${config_path}/log4rs_miner.yml" --base-path=${base_path} > $exe_path/taiji_miner_command.sh
chmod +x $exe_path/minotaiji_miner_command.sh

open -a terminal $exe_path/minotaiji_miner_command.sh
echo
