...
ASSET_URL=$(curl -fsSL https://api.github.com/repos/tari-project/tari/releases/latest \
  | jq -r '
      .assets[]
      | select(.name | test("minotari_ledger_wallet-nanosplus.*\\.zip$"))
      | .browser_download_url
    ')

if [[ -z "$ASSET_URL" || "$ASSET_URL" == "null" ]]; then
  echo "❌ Could not find nanosplus release asset."
  exit 1
fi
...
APP_JSON=$(find . -name "app_nanosplus.json" | head -n 1)

if [[ -z "$APP_JSON" ]]; then
  echo "❌ app_nanosplus.json not found after unzip."
  exit 1
fi

echo
echo "🔐 Installing app onto Ledger Nano S Plus..."
echo "👉 Ensure:"
