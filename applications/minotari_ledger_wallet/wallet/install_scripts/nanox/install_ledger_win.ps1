...
$Asset = $Release.assets |
    Where-Object { $_.name -match "minotari_ledger_wallet-nanox.*\.zip" } |
    Select-Object -First 1

if (-not $Asset) {
    Write-Error "Could not find nanox release asset."
}
...
$appJson = Get-ChildItem -Recurse -Filter "app_nanox.json" | Select-Object -First 1

if (-not $appJson) {
    Write-Error "app_nanox.json not found after extraction."
}

Write-Host ""
Write-Host "🔐 Installing app onto Ledger Nano X..." -ForegroundColor Yellow
Write-Host "👉 Ensure:"
