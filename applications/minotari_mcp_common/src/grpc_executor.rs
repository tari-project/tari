...
impl GrpcExecutor {
    ...
    async fn execute_node_method(
        ...
        let result = match method_info.name.as_str() {
            ...
            "ListHeaders" => {
                let from_height = parameters.get("from_height")
                    .and_then(|v| v.as_u64());
                let to_height = parameters.get("to_height")
                    .and_then(|v| v.as_u64());
    ...
    async fn execute_wallet_method(
        ...
        let result = match method_info.name.as_str() {
            ...
            "Transfer" => {
                ...
                let fee_per_gram = parameters.get("fee_per_gram")
                    .and_then(|v| v.as_u64());
            ...
            "CreateBurnTransaction" => {
                ...
                let message = parameters.get("message")
                    .and_then(|v| v.as_str());
                let claim_public_key = parameters.get("claim_public_key")
                    .and_then(|v| v.as_str());
                client.create_burn_transaction(amount, fee_per_gram, message, claim_public_key).await
            }
            "CoinSplit" => {
                let amount = parameters.get("amount")
                    .and_then(|v| v.as_u64())
            ...
            "ImportUtxos" => {
                let outputs = parameters.get("outputs")
                    .and_then(|v| v.as_array())
                    .ok_or_else(|| McpError::invalid_request("Missing 'outputs' parameter"))?
                    .iter()
                    .map(|v| v.as_str().unwrap_or("").to_string())
                    .collect();
                let source_public_keys = parameters.get("source_public_keys")
                    .and_then(|v| v.as_array())
