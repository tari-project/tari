...
impl ConversionRegistry {
        ...
        C: ParameterConverter + 'static,
    {
        ...
        let boxed_converter = Box::new(move |parameters: Value| {
            ...
            tokio::task::block_in_place(|| {
                tokio::runtime::Handle::current().block_on(converter.convert(parameters))
            })
        });
    ...
    pub fn convert(&self, method_name: &str, parameters: Value) -> Result<Box<dyn prost::Message + Send>, ConversionError> {
...
impl JsonParameterExtractor for Value {
    ...
    fn get_required_u64(&self, method: &str, key: &str) -> Result<u64, ConversionError> {
        match self.get(key) {
            Some(Value::Number(n)) => {
                n.as_u64().ok_or_else(|| ConversionError::InvalidParameterType {
                    method: method.to_string(),
                    param: key.to_string(),
