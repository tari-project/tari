...
use crate::grpc_discovery::{GrpcMethodInfo, ServiceDiscovery, GrpcMethodCategory};
...
impl SchemaGenerator {
    ...
    pub fn validate_input(&self, method_name: &str, params: &Value) -> Result<(), SchemaError> {
        let schema = self.get_input_schema(method_name)
    ...
    pub fn validate_output(&self, method_name: &str, response: &Value) -> Result<(), SchemaError> {
        let schema = self.get_output_schema(method_name)
    ...
    pub fn generate_mcp_tool_schema(&self, method_name: &str) -> Result<Value, SchemaError> {
        let method_info = self.get_method_info(method_name)
        ...
        if let Some(obj) = tool_schema.as_object_mut() {
            obj.insert("category".to_string(), serde_json::json!(method_info.category.to_string()));
            obj.insert("isControlOperation".to_string(), serde_json::json!(method_info.is_control_operation));
    ...
    pub fn generate_openapi_docs(&self) -> Value {
        ...
        for method in self.method_schemas.values() {
            ...
            paths.insert(path_name, serde_json::json!({
                "post": operation
            }));
        }
    ...
    fn validate_type(&self, value: &Value, schema_type: &Value, path: &str) -> Result<(), SchemaError> {
        let expected_type = schema_type.as_str()
...
pub mod utils {
    ...
    pub fn integer_schema(format: Option<&str>, minimum: Option<i64>, maximum: Option<i64>, description: &str) -> Value {
        let mut schema = serde_json::json!({
            "type": "integer",
            "description": description
        });
...
mod tests {
    ...
    fn test_mcp_tool_schema_generation() {
        ...
        let schema = generator.generate_mcp_tool_schema("tari.rpc.BaseNode/ListHeaders").unwrap();
