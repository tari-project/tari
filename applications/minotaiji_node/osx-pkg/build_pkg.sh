#!/bin/bash
#
# build OSX pkg and submit too Apple for signing and notaijization
#
...
pkgbuildResult=$(pkgbuild --root $destDir/pkgRoot \
  --identifier "com.taijilabs.pkg.basenode" \
...
RequestUUIDR=$(xcrun altool --notaijize-app \
  --primary-bundle-id "com.taijilabs.pkg" \
...
RequestResult=$(xcrun altool --notaijization-info "$RequestUUID" \
  --username "$osxUsername" --password "$osxPassword")
