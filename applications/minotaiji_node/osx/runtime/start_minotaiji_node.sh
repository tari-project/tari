...
then
    ...
    "${exe_path}/minotaiji_node" --create_id --init --config "${config_path}/config.toml" --log_config "${config_path}/log4rs_base_node.yml" --base-path ${base_path}
else
...
then
    ...
    "${exe_path}/minotaiji_node" --init --config "${config_path}/config.toml" --log_config "${config_path}/log4rs_base_node.yml" --base-path ${base_path}
else
...
echo "${exe_path}/minotaiji_node" --config="${config_path}/config.toml" --log_config="${config_path}/log4rs_base_node.yml" --base-path="${base_path}" > $exe_path/minotaiji_node_command.sh
chmod +x $exe_path/minotaiji_node_command.sh

open -a terminal $exe_path/minotaiji_node_command.sh
echo
