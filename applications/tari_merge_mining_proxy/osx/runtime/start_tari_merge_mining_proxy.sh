...
# Run
chmod +x "${exe_path}/start_tari_merge_mining_proxy.sh"
echo "${exe_path}/tari_merge_mining_proxy" --config="${config_path}/config.toml" --log_config "${config_path}/log4rs_merge_mining_proxy.yml" --base-path "${base_path}" > "$exe_path/start_tari_merge_mining_proxy.sh"
...
then
    ...
    open -a terminal "${exe_path}/start_tari_merge_mining_proxy.sh"
fi
echo
