...
impl ReceiveTab {
    ...
    where B: Backend {
        ...
        const ITEM_01: &str = "Taiji Address:     ";
        const ITEM_02: &str = "Node ID:        ";
        const ITEM_03: &str = "Network Address: ";
        const ITEM_04: &str = "Emoji ID:       ";

        // Taiji address
        let taiji_address_text = Spans::from(vec![
            Span::styled(ITEM_01, Style::default().fg(Color::Magenta)),
            Span::styled(
                app_state.get_identity().taiji_address.clone(),
                Style::default().fg(Color::White),
            ),
        ]);
        let paragraph = Paragraph::new(taiji_address_text).block(Block::default());
