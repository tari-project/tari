...
use log::*;
use minotaiji_wallet::output_manager_service::UtxoSelectionCriteria;
    ...
    where B: Backend {
        ...
        let to_input = Paragraph::new(self.to_field.as_ref())
            ...
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .title("(T)o (Taiji Address or Emoji ID) :"),
            );
            ...
            if 'n' == c {
                ...
                match self.confirmation_dialog {
                    ...
                    Some(ConfirmationDialogType::StealthAddress) => {
                        if 'y' == c {
                            let amount = if let Ok(v) = self.amount_field.parse::<MicroMinotaiji>() {
                                v
                            } else {
                                if self.selected_unique_id.is_none() {
                                    self.error_message =
        ...
        match c {
            ...
            's' | 'o' | 'x' => {
                if self.to_field.is_empty() {
                    self.error_message =
                        Some("Destination Taiji Address/Emoji ID\nPress Enter to continue.".to_string());
                    return;
                }
                ...
                if self.amount_field.parse::<MicroMinotaiji>().is_err() && self.selected_unique_id.is_none() {
                    self.error_message =
                        Some("Amount should be a valid amount of Minotaiji\nPress Enter to continue.".to_string());
                    return;
                }
