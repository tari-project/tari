...
use taiji_core::transactions::{taiji_amount::MicroMinotaiji, transaction_components::TemplateType};
        ...
        match self.confirmation_dialog {
            ...
            Some(ConfirmationDialogType::Normal) => match c {
                ...
                'y' => {
                    ...
                    let fee_per_gram = if let Ok(fee_per_gram) = MicroMinotaiji::from_str(self.fee_per_gram.as_str()) {
                        fee_per_gram
                    } else {
                        self.confirmation_dialog = None;
                        self.error_message =
                                ...
                                Ok(data) => match data.status() {
                                    StatusCode::OK => match data.bytes().await {
                                        Ok(bytes) => {
                                            ...
                                            TaijiEngineHashDomain::add_domain_separation_tag(&mut hasher, "Template");
    ...
    fn on_key(&mut self, app_state: &mut AppState, c: char) {
        ...
        match c {
            ...
            's' => {
                ...
                if self.fee_per_gram.parse::<MicroMinotaiji>().is_err() {
                    self.error_message =
                        Some("Fee-Per-Gram should be a valid amount of Minotaiji\nPress Enter to continue.".to_string());
                    return;
                }
