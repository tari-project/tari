...
echo "${exe_path}/minotaiji_console_wallet" ${init_flag} --config="${config_path}/config.toml" --log_config="${config_path}/log4rs_console_wallet.yml" --base-path=${base_path} > $exe_path/minotaiji_console_wallet_command.sh
chmod +x $exe_path/minotaiji_console_wallet_command.sh

open -a terminal $exe_path/minotaiji_console_wallet_command.sh
echo
