...
async function doSend (senderClient, receiverClient, amount, numberTransactions = 1) {
  let receiverInfo = await receiverClient.identify()
  let startingBalance = parseInt((await senderClient.getBalance()).available_balance)
  ...
  for (let i = 0; i < 20 * 60; i++) {
    ...
    console.log(`Waiting for new balance (spender) ${newBalance.available_balance} to be more than required amount: ${startingBalance}. Pending ${newBalance.pending_balance}`)
