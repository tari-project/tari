...
//! # Configuration of taiji applications
//!
//! Taiji application consist of `common`, `base_node`, `wallet` and `application` configuration sections.
//! All taiji apps follow traits implemented in this crate for ease and automation, for instance managing config files,
...
//! - [global] - load GlobalConfig for Taiji
//! - [loader] - build and load configuration modules in a taiji-way
//! - [utils] - utilities for working with configuration
//!
//! ## Configuration file
//!
//! The taiji configuration file (config.yml) is intended to be a single config file for all Taiji desktop apps to use
//! to pull configuration variables, whether it's a testnet base node; wallet; validator node etc.
//!
//! The file lives in ~/.taiji by default and has sections which will allow a specific app to determine
