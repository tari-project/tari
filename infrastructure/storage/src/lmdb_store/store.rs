...
impl LMDBStore {
    ...
    /// Produce a compacted copy of the LMDB environment at the given destination path.
    pub fn compact_to<P: AsRef<Path>>(&self, dest: P) -> Result<(), LMDBError> {
        let path_str = dest.as_ref().to_str().ok_or(LMDBError::InvalidPath)?;
        self.env.copy(path_str, lmdb_zero::copy::COMPACT)?;
        Ok(())
    }
