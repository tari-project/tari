...
use crate::keyvalue_store::BatchWrite;
use crate::keyvalue_store::{DataStore, DatastoreError};
...
impl DataStore for LMDBStore {
    ...
    fn get_raw(&self, key: &[u8]) -> Result<Option<Vec<u8>>, DatastoreError> {
        ...
        match accessor.get::<[u8], [u8]>(&self.curr_db, key).to_opt() {
            ...
            Err(e) => Err(DatastoreError::GetError(format!(
                "LMDB get error: {}",
                e.to_string()
            ))),
        }
...
mod test {
    ...
    fn path_must_exist() {
        ...
        match builder.set_mapsize(1).set_path("./tests/not_here/").build() {
            Err(DatastoreError::InternalError(s)) => {
                assert_eq!(s, "LMDB Error: No such file or directory")
            }
            _ => panic!(),
        }
    }
    ...
    fn overflow_db() {
        ...
        match store.put_raw(b"key2", make_vector(600_000)).unwrap_err() {
            DatastoreError::InternalError(s) => {
                assert_eq!(
                    s,
                    "LMDB Error: MDB_MAP_FULL: Environment mapsize limit reached"
                );
            }
            err => {
                println!("{:?}", err);
