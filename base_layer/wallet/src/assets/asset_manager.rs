 ...
 impl<T:OutputManagerBackend + 'static, TPersistentKeyManager: PersistentKeyManager> AssetManager<T, TPersistentKeyManager> {
    ...
    pub async fn list_owned(&self) -> Result<Vec<Asset>, WalletError>{
        let outputs = self.output_database.fetch_with_features(OutputFlags::ASSET_REGISTRATION).await.map_err(|err| WalletError::OutputManagerError(err.into()))?;
         ...
         let assets: Vec<Asset> = outputs.into_iter().map(|unblinded_output| {
     ...
     pub async fn create_registration_transaction(&mut self, name: String) -> Result<(TxId, Transaction), WalletError>{
         ...
         let output = self.output_manager.create_output_with_features(
             0.into(), OutputFeatures::for_asset_registration(metadata_bin, public_key), Some(public_key_bytes), None).await?;
     ...
     pub async fn create_minting_transaction(&mut self, asset_public_key: PublicKey, asset_owner_commitment: Commitment, unique_ids: Vec<Vec<u8>>) -> Result<(TxId, Transaction), WalletError> {
        ...
        // TODO: generate proof of ownership
         for id  in unique_ids {
             let output = self.output_manager.create_output_with_features(0.into(), OutputFeatures::for_minting(vec![], asset_public_key.clone(), asset_owner_commitment.clone()), Some(id), Some(asset_public_key.clone())).await?;
             outputs.push(output);
         }

         let (tx_id, transaction) = self.output_manager.create_send_to_self_with_output(0.into(), outputs, 100.into()).await?;
         Ok((tx_id, transaction))
     }
}
...
fn convert_to_asset(unblinded_output: DbUnblindedOutput) -> Result<Asset, WalletError> {
    if unblinded_output.unblinded_output.features.metadata.is_empty() {
        ...
        return Ok(Asset::new("<Invalid metadata:empty>".to_string(), unblinded_output.status.to_string(), unblinded_output.unblinded_output.features.asset.as_ref().map(|a| a.public_key.clone()).unwrap(), unblinded_output.commitment)) ;
    }
    ...
    Ok(Asset::new(metadata.name, unblinded_output.status.to_string(),unblinded_output.unblinded_output.features.asset.as_ref().map(|a| a.public_key.clone()).unwrap(), unblinded_output.commitment ))
}
