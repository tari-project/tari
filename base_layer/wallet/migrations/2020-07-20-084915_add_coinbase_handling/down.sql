...
ALTER TABLE pending_transaction_outputs
    RENAME TO pending_transaction_outputs_old;
CREATE TABLE pending_transaction_outputs (
    tx_id      INTEGER PRIMARY KEY NOT NULL,
    short_term INTEGER             NOT NULL,
    timestamp  DATETIME            NOT NULL
);
INSERT INTO pending_transaction_outputs (tx_id, short_term, timestamp)
SELECT tx_id, short_term, timestamp
FROM pending_transaction_outputs_old;
...
INSERT INTO completed_transactions (tx_id, source_public_key, destination_public_key, amount, fee, transaction_protocol,
                                    status, message, timestamp, cancelled, direction)
SELECT tx_id,
       source_public_key,
