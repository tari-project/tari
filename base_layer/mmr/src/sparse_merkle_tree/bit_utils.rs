...
#[inline]
pub fn height_key(key: &NodeKey, height: usize) -> Option<NodeKey> {
    ...
    bytes.get_mut(0..height / 8)?.copy_from_slice(key.get(0..height / 8)?);
    // The height/8th byte is only partially copied, so mask the byte & 11100000, where the number of 1s is
    // height % 8
    *bytes.get_mut(height / 8)? = key.get(height / 8)? & !(0xff >> (height % 8));
    Some(result)
}
...
pub fn traverse_direction(
    ...
    let dir = bit_to_dir(
        get_bit(child_key.as_slice(), parent_height)
            .ok_or(SMTError::IllegalKey("parent does not exist".to_string()))?,
    );
    Ok(dir)
}
...
mod test {
    ...
    fn get_bits() {
        ...
        for i in 0..16 {
            assert_eq!(get_bit(&val, i).unwrap(), (i + 1) % 2);
        }
        for i in 16..24 {
            assert_eq!(get_bit(&val, i).unwrap(), 0);
        }
        for i in 24..32 {
            assert_eq!(get_bit(&val, i).unwrap(), 1);
        }
    }
}
