...
/// signature per committee member, so this is an upper bound for any committee size. It is checked at deserialization
/// boundaries so that the work done before any signature is verified is bounded.
