...
#[no_mangle]
pub unsafe extern "C" fn public_key_add(
    a: *mut TariPublicKey,
    b: *mut TariPublicKey,
    error_out: *mut c_int,
) -> *mut TariPublicKey {
    ...
    if a.is_null() {
        error = LibWalletError::from(InterfaceError::NullError("key (a) is null".to_string())).code;
        ptr::swap(error_out, &mut error as *mut c_int);
        return ptr::null_mut();
    }

    if b.is_null() {
        error = LibWalletError::from(InterfaceError::NullError("key (b) is null".to_string())).code;
        ptr::swap(error_out, &mut error as *mut c_int);
        return ptr::null_mut();
    }

    Box::into_raw(Box::new((*a).clone() + (*b).clone()))
}
...
pub unsafe extern "C" fn private_key_add(
    a: *mut TariPrivateKey,
    b: *mut TariPrivateKey,
    error_out: *mut c_int,
) -> *mut TariPrivateKey {
    let mut error = 0;
    ptr::swap(error_out, &mut error as *mut c_int);
...
#[no_mangle]
pub unsafe extern "C" fn create_shared_secret(
    k: *mut PrivateKey,
    pk: *mut PublicKey,
    error_out: *mut c_int,
) -> *mut PublicKey {
    ...
    if k.is_null() {
        error = LibWalletError::from(InterfaceError::NullError("private key is null".to_string())).code;
        ptr::swap(error_out, &mut error as *mut c_int);
        return ptr::null_mut();
    }

    if pk.is_null() {
        error = LibWalletError::from(InterfaceError::NullError("public key is null".to_string())).code;
        ptr::swap(error_out, &mut error as *mut c_int);
        return ptr::null_mut();
    }

    Box::into_raw(Box::new(PublicKey::shared_secret(&(*k), &(*pk))))
}
...
/// a memory leak
#[no_mangle]
pub unsafe extern "C" fn tari_hasher_create(label: *const c_char, error_out: *mut c_int) -> *mut TariHasher {
    let mut error = 0;
    ptr::swap(error_out, &mut error as *mut c_int);

    if label.is_null() {
        error = LibWalletError::from(InterfaceError::NullError("label".to_string())).code;
        ptr::swap(error_out, &mut error as *mut c_int);
        return ptr::null_mut();
    }
    ...
    let label = match CStr::from_ptr(label).to_str() {
        Ok(s) => s.to_owned(),
        _ => {
            error = LibWalletError::from(InterfaceError::PointerError("label".to_string())).code;
            ptr::swap(error_out, &mut error as *mut c_int);
            return ptr::null_mut();
        },
    };

    Box::into_raw(Box::new(DomainSeparatedHasher::<Blake256, GenericHashDomain>::new(
        label,
    )))
}
...
/// a memory leak
#[no_mangle]
pub unsafe extern "C" fn tari_hasher_update(hasher: *mut TariHasher, v: *mut ByteVector, error_out: *mut c_int) {
    let mut error = 0;
    ptr::swap(error_out, &mut error as *mut c_int);

    if hasher.is_null() {
        error = LibWalletError::from(InterfaceError::NullError("hasher instance is null".to_string())).code;
        ptr::swap(error_out, &mut error as *mut c_int);
        return;
    }

    if v.is_null() {
        error = LibWalletError::from(InterfaceError::NullError("byte vector is null".to_string())).code;
        ptr::swap(error_out, &mut error as *mut c_int);
        return;
    }

    (*hasher).update((*v).0.to_vec());
}
...
#[no_mangle]
pub unsafe extern "C" fn tari_hasher_finalize(hasher: *mut TariHasher, error_out: *mut c_int) -> *mut ByteVector {
    let mut error = 0;
    ptr::swap(error_out, &mut error as *mut c_int);
    ...
    let h = Box::from_raw(hasher).finalize().into_vec();

    Box::into_raw(Box::new(ByteVector(h)))
}
...
/// None
#[no_mangle]
pub unsafe extern "C" fn tari_hasher_destroy(hasher: *mut TariHasher) {
    if !hasher.is_null() {
        Box::from_raw(hasher);
    }
}
...
mod test {
    ...
    fn test_hasher() {
        unsafe {
            ...
            // test values for further comparison
            let raw_label = "com.tari.test_label";
            let value0 = "test value";
            let value1 = "another test value";

            // ----------------------------------------------------------------------------
            // FFI hashing

            // initializing hasher
            let label = CString::new(raw_label).unwrap().into_raw() as *const c_char;
            let hasher = tari_hasher_create(label, error_ptr);
            assert_eq!(error, 0);

            // first update
            tari_hasher_update(
                hasher,
                Box::into_raw(Box::new(ByteVector(value0.as_bytes().to_vec()))),
                error_ptr,
            );
            assert_eq!(error, 0);
            ...
            tari_hasher_update(
                hasher,
                Box::into_raw(Box::new(ByteVector(value1.as_bytes().to_vec()))),
                error_ptr,
            );
            assert_eq!(error, 0);

            // finalization
            let hash_from_ffi = tari_hasher_finalize(hasher, error_ptr);
            assert_eq!(error, 0);

            // WARNING: `tari_hasher_finalize()` consumes the hasher, no need to free afterwards.
            // tari_hasher_destroy(hasher);

            // ----------------------------------------------------------------------------
            // comparing to native hash

            let good_native_hash = DomainSeparatedHasher::<Blake256, GenericHashDomain>::new(raw_label)
                .chain(value0.as_bytes())
                .chain(value1.as_bytes())
                .finalize()
                .into_vec();

            assert_eq!(good_native_hash, (*hash_from_ffi).0);

            let invalid_native_hash = DomainSeparatedHasher::<Blake256, GenericHashDomain>::new(raw_label)
                .chain(value0.as_bytes())
                .chain("where did it come from?".as_bytes())
                .chain(value1.as_bytes())
                .finalize()
                .into_vec();

            assert_ne!(invalid_native_hash, (*hash_from_ffi).0);
        }
    }
    ...
    fn test_shared_secret() {
        unsafe {
            ...
            // Alice
            let k0 = private_key_generate();
            let pk0 = public_key_from_private_key(k0, error_ptr);
            assert_eq!(error, 0);

            // Bob
            let k1 = private_key_generate();
            let pk1 = public_key_from_private_key(k1, error_ptr);
            assert_eq!(error, 0);

            // Alice's share of the secret
            let k0s = create_shared_secret(k0, pk1, error_ptr);
            assert_eq!(error, 0);

            // Bob's share of the secret
            let k1s = create_shared_secret(k1, pk0, error_ptr);
            assert_eq!(error, 0);
            ...
            // ----------------------------------------------------------------------------
            // just in case if Charlie is bored

            let k2 = private_key_generate();
            let pk2 = public_key_from_private_key(k2, error_ptr);
            assert_eq!(error, 0);

            let k2s = create_shared_secret(k2, pk0, error_ptr);
            assert_eq!(error, 0);
            assert_ne!((*k2s), (*k1s));

            let k2s = create_shared_secret(k2, pk1, error_ptr);
            assert_eq!(error, 0);
            assert_ne!((*k2s), (*k0s));

            private_key_destroy(k0);
            private_key_destroy(k1);
            private_key_destroy(k2);

            public_key_destroy(pk0);
            public_key_destroy(pk1);
            public_key_destroy(pk2);

            public_key_destroy(k0s);
            public_key_destroy(k1s);
            public_key_destroy(k2s);
        }
    }
    ...
    fn test_sum_of_private_keys() {
        unsafe {
            ...
            // native computation
            let expected_key = (*k0).clone() + (*k1).clone();

            // computed via FFI
            let sum_from_ffi0 = private_key_add(k0, k1, error_ptr);
            let sum_from_ffi1 = private_key_add(k0, k2, error_ptr);

            assert_eq!(expected_key, (*sum_from_ffi0));
            assert_eq!(error, 0);
            assert_ne!(expected_key, (*sum_from_ffi1));
            assert_eq!(error, 0);

            private_key_destroy(k0);
            private_key_destroy(k1);
            private_key_destroy(k2);
            private_key_destroy(sum_from_ffi0);
            private_key_destroy(sum_from_ffi1);
        }
    }

    #[test]
    fn test_sum_of_public_keys() {
        unsafe {
            let mut error = 0;
            let error_ptr = &mut error as *mut c_int;
            ...
            // native computation
            let expected_key = (*pk0).clone() + (*pk1).clone();

            // computed via FFI
            let sum_from_ffi0 = public_key_add(pk0, pk1, error_ptr);
            let sum_from_ffi1 = public_key_add(pk0, pk2, error_ptr);
            ...
            public_key_destroy(pk1);
            public_key_destroy(pk2);
            public_key_destroy(sum_from_ffi0);
            public_key_destroy(sum_from_ffi1);
        }
    }
}
