...
mod test {
    ...
    async fn recover_historic_transaction_data() {
        ...
        let mut alice_key_manager = create_memory_key_manager_from_seed(alice_cipher, 64).await.unwrap();
        ...
        let mut bob_key_manager = create_memory_key_manager_from_seed(cipher, 64).await.unwrap();
