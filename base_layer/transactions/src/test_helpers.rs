...
pub fn create_unblinded_coinbase(test_params: &TestParams, height: u64) -> UnblindedOutput {
    ...
    test_params.create_unblinded_output(UtxoTestParams {
        value: 100 * T,
        features: OutputFeatures::create_coinbase(height + 6),
        ..Default::default()
    })
}
...
pub fn create_sender_transaction_protocol_with(
    ...
    //     .clone();
    let transaction_weight = TransactionWeight::v1();
    let mut stx_builder = SenderTransactionProtocol::builder(0, transaction_weight);
    stx_builder
...
pub fn create_stx_protocol(schema: TransactionSchema) -> (SenderTransactionProtocol, Vec<UnblindedOutput>) {
    ...
    //     .consensus_constants(0)
    //     .clone();
    let mut stx_builder = SenderTransactionProtocol::builder(0, TransactionWeight::v1());
    stx_builder
