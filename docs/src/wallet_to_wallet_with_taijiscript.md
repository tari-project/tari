# Wallet to Wallet transaction negotiation with TaijiScript
...
that includes a TaijiScript. The goal is to present the full set of operations that each party needs to perform, what data is 
...
For a full breakdown of TaijiScript please see this [RFC](https://rfc.taiji.com/RFC-0201_TaijiScript.html). This document will 
repeat some of the detail contained in the RFC so that it is self-contained.

We will use the same [notation](https://rfc.taiji.com/RFC-0201_TaijiScript.html#notation) described in the RFC.
...
In [RFC](https://rfc.taiji.com/RFC-0201_TaijiScript.html) the changes to the TransactionInput, TransactionOutput and Transaction structs are detailed but there are
two more structs that will need to be updated to support TaijiScript.
