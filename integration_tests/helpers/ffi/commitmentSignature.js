...
class CommitmentSignature {
  ...
  static createFromObject(obj) {
    ...
    return CommitmentSignature.createFromHex(
  ...
  static createFromHex(public_nonce_hex, u_hex, v_hex) {
    let public_nonce_bytes = ByteVector.fromBytes(
      public_nonce_hex.toString("hex")
    ).getPtr();
    ...
    result.pointerAssign(
      InterfaceFFI.commitmentSignatureCreateFromBytes(
        public_nonce_bytes,
        u_bytes,
        v_bytes
      )
